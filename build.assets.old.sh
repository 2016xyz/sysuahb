...
CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -ldflags "-s -w -extldflags -static -extldflags -static -X 'github.com/djylb/nps/lib/install.BuildTarget=win7'" -o ${NPC_BIN}.exe ./cmd/npc/npc.go

tar -czvf windows_386_client_old.tar.gz ${NPC_BIN}.exe conf/sysficb.conf conf/multi_account.conf


CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags "-s -w -extldflags -static -extldflags -static -X 'github.com/djylb/nps/lib/install.BuildTarget=win7'" -o ${NPC_BIN}.exe ./cmd/npc/npc.go

tar -czvf windows_amd64_client_old.tar.gz ${NPC_BIN}.exe conf/sysficb.conf conf/multi_account.conf


CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags "-s -w -extldflags -static -extldflags -static -X 'github.com/djylb/nps/lib/install.BuildTarget=win7'" -o ${NPS_BIN}.exe ./cmd/nps/nps.go

tar -czvf windows_amd64_server_old.tar.gz conf/sysuahb.conf web/views web/static ${NPS_BIN}.exe


CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -ldflags "-s -w -extldflags -static -extldflags -static -X 'github.com/djylb/nps/lib/install.BuildTarget=win7'" -o ${NPS_BIN}.exe ./cmd/nps/nps.go
