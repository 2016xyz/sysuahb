...
- **统一代理用户名路由重构**：新增标签维度与统一解析器，HTTP 与 SOCKS5 共用 `RouteParser → ClientSelector → StickyStore` 同一条链路
  - `auto`：全部可用客户端随机，每连接重选，不缓存
  - `<clientId>`：固定客户端，离线直接失败，不 fallback
  - `<tag>.auto`：仅在该标签的可用客户端中随机，每连接重选，不缓存
  - `<stickyKey>-auto[-<ttl>]`：全量粘滞随机
  - `<stickyKey>.<tag>-auto[-<ttl>]`：按标签粘滞随机
  - TTL 支持 `m`/`h`/`d`，范围 1m ~ 24h，非法用户名 / 标签 / TTL 一律拒绝，不猜测不降级
- 粘滞缓存改为 `CacheKey → ClientID`（不再缓存客户端指针），键含 `UnifiedProxyID + 完整用户名`，避免不同统一代理实例串缓存；命中时重新校验客户端是否存在 / 在线 / 可用 / 是否仍带有所需标签，任一不满足立即失效并重新选择
- 统一代理密码强制非空：前端提交校验 + 后端任务创建/编辑校验 + 连接认证校验（HTTP 407 / SOCKS5 拒绝）
- 出口客户端按**连接**固定：HTTP Keep-Alive、HTTPS CONNECT、SOCKS5 生命周期内不切换；Sticky TTL 到期仅影响下一条新连接
- 文档：README 中英文同步补充「客户端标签」与「统一代理」章节，并修正中英文互链
- 测试：新增/重写统一代理解析、选择器、粘滞缓存与 HTTP/SOCKS5 端到端用例
- 修复：`<key>-auto-` 这类「TTL 标记后为空」的用户名会被静默当作无 TTL 接受；现在与文档一致直接拒绝，并补充边界用例

### v0.34.7-test2 (2026-09-19)
- 文档：README 中英文同步补充「客户端标签」与「统一代理」新语法，修正中英文互链
- CI：测试版自动标记为 pre-release，不覆盖 Docker 的 `latest` 镜像标签

### v0.34.7-test3 (2026-09-19)
- 修复：`<key>-auto-` 边界用例 —— TTL 标记后为空时直接拒绝，不再静默降级为默认 TTL

### v0.34.7-test4 (2026-09-20)
- 新增**外部代理节点**：统一代理出口不再局限于 NPS Client，HTTP / SOCKS5 上游代理与 NPS Client 进入**同一个统一出口池**（`EgressSelector`）
  - `RouteParser` 只解析路由规则，不再关心出口是 Client 还是 Proxy
  - 粘滞缓存改为缓存 `EgressRef{Type,ID}`（client / proxy），命中后重新校验存在性 / Enabled / Available / 标签 / 协议
  - 协议匹配：HTTP 请求只选支持 HTTP 的出口，SOCKS5 请求只选支持 SOCKS5 的出口
  - 外部代理健康检测：真实经代理访问 `https://www.gstatic.com/generate_204`，204 才算成功，记录延迟 / 连续失败 / 最后错误；默认正常 5m、异常 1m、超时 10s、连续失败 3 次标记 Unavailable、成功 1 次恢复，最大并发 20
  - 新增「代理节点」页面（增删改 / 启用禁用 / 批量导入 / 批量操作 / 搜索筛选）与「设置」页面（健康检测与 Sticky 全局参数）
  - 代理节点与设置持久化于 `conf/proxies.json`、`conf/unified.json`；列表 API 不返回明文密码，编辑时密码留空表示保持原密码
- 修复：安全审计 —— 统一代理并发竞争与自死锁

### v0.34.7-test5 (2026-09-21)
- 客户端去除二进制特征指纹：模块路径、进程名、服务名、配置路径、日志与更新地址不再出现可识别字样，功能与旧客户端保持兼容
- 修复：`lib/conn` 接收缓冲区（`rbs`）在 `Close` / `Read` 并发访问下的数据竞争
- 修复：客户端入口改名后遗留的构建脚本与 `Dockerfile` 路径
- 修复：`build.sh` / `build.assets.old.sh` 中 `././cmd/...` 重复路径
- CI：串行化发布上传任务（`needs: build_assets`），消除多处 `action-gh-release` 并发创建 release 的竞争
- 清理：随包示例配置中的 `npc` / `nps` 字样

### v0.34.7-test6 (2026-09-21)
- 修复：代理节点页面路由前缀错误 —— Beego `AutoRouter` 注册的是 `/proxynode/*`，而视图与前端全部指向 `/proxy_node/*`，导致统一代理菜单下所有页面与 AJAX 请求 404
- 修复：安装脚本版本号解析 —— 形如 `0.34.7-test6` 的**预发布版本号不补 `v` 前缀**，导致按版本下载 404 后又**静默回退 `latest`**，最终装成稳定版 `v0.34.7`（比用户要求的版本旧，且无测试版功能）。现在 `0.34.7-test6` 会正确归一化为 `v0.34.7-test6`
- 修复：固定版本安装时不再回退 `releases/latest/download` —— 版本号写错（如不存在的 `0.34.8`）会**直接报错退出**，不会静默装成别的版本
- 文档：新增 [测试版安装指南](docs/install-test.md)，并在稳定版安装文档中明确 `latest` = 稳定版 `v0.34.7`、测试版必须显式指定版本号

### v0.34.7-test7 (2026-09-21)
- 安装脚本：版本号归一化支持预发布后缀 —— `0.34.7-testN` 现在会补成 `v0.34.7-testN`（此前 404 后**静默回退 `latest`**，把测试版装成稳定版 `v0.34.7`）
- 安装脚本：**固定版本时不再回退 `releases/latest`** —— 版本号不存在（如 `0.34.8`）直接报错退出，杜绝"装成别的版本却显示成功"
- 安装脚本：非法版本号格式直接拒绝，并提示正确写法
...
### v0.34.7-test9 (2026-09-22)
- **代码审计修复**（统一代理 / 全能代理）：修正 12 处缺陷，新增 5 个回归测试文件
  - 数据竞争：拨号路径未加锁读取 `Username`/`Password`/`Host`/`Scheme`/`Name`，与 `UpdateConfig` 写入竞争
    → 新增 `Credentials()`/`HostValue()`/`NameValue()` 等访问器；`Addr()` 改为加锁读
  - 死锁：`String()` 持读锁时又调用加锁的 `Addr()`，写者排队即自锁 → 地址改为锁内内联格式化
  - `AddTags` 丢失更新：读-改-写未在同一临界区，并发加标签会互相覆盖（实测 50 个仅存 13 个）
  - 禁用节点状态污染：在途探测可把已禁用节点改写为 Available/Unavailable（含 `ChangeStatus` 的 TOCTOU）
  - **隧道节点被编辑破坏**：`/proxynode/edit` 会清空 SS/VMess/Trojan 节点的 Scheme，使其沦为无协议僵尸节点
    → 移除 `ProxyConfig` 中从未被表单填写的隧道字段；该端点拒绝隧道节点
  - 列表职责混用：「代理节点」页混入隧道节点 → 已按职责分离（隧道节点只在全能代理页）
  - 「删除无效代理」误选已禁用节点 → 改为严格匹配 `Status = Unavailable AND 失败数 >= 阈值`
  - **SSRF**：Clash 订阅地址未校验，可让服务器请求内网 / 云元数据端点
    → 仅允许 http/https，解析后拒绝所有非公网地址，并以已验证 IP 直连（防 DNS 重绑定）
  - 批量导入去重失效：「已存在」集合用空协议键构建，与 `http`/`socks5` 键永不匹配，重复导入不跳过
...
### v0.34.7 (上游 2026-03-12 · 本仓库 tag 2026-09-15)
- 优化代码结构
- 优化代码性能
- 调整默认配置文件
- 更新相关依赖包
...
#### 本仓库改动（2026-09-14）
- 随机进程名：服务端 `sysuahb` / 客户端 `sysficb`，Linux 一键安装脚本每次安装生成随机名
- 新增统一代理：按登录用户名路由出口——`auto` 每连接随机、纯数字固定客户端 ID、`xxx-auto` 粘滞默认 10 分钟、`xxx-auto-5m/30m/2h/1d` 自定义时长，其他用户名拒绝
- 统一代理连接密码强制校验（HTTP 407 / SOCKS5 拒绝）
...
- 统一代理：粘滞缓存新增后台过期清理（每分钟清除过期条目），避免长期运行时过期条目滞留内存
- 测试：补充统一代理用户名解析与粘滞缓存逻辑的单元测试

#### 本仓库改动（2026-09-15）
- 安装脚本：版本号自动补 `v` 前缀（如 `0.34.7` → `v0.34.7`），避免按版本下载时 404（此前靠 latest 兜底成功）
